...
import { SystemUser, OrgList_Type , Leave} from "./type";

export const wFHApplicationSignupRequest = async ({firstName, lastName, email, password, dob}: SystemUser) => {
...
export const wFHApplicationFetch = async ({orgList, email}: OrgList_Type) => {
    ...
    try{
        ...
        const application = await axios.post(URL, JSON.stringify({orgList, email}), {headers})
...
export const acceptedLeaveReq = async ({_id}: Leave) => {
    ...
    try{
        ...
        const api = await axios.post(URL, JSON.stringify({_id, statusValue: 1}), {headers})
        return api.data;
...
export const rejectedLeaveReq = async ({_id}: Leave) => {
    ...
    try{
        ...
        const api = await axios.post(URL, JSON.stringify({_id, statusValue: 2}), {headers})
        return api.data;

...
export const SystemUserDashBoardRequest = async (token: string) => {
    ...
    try{
        ...
        const post_dashboard_api = await axios.post(URL, JSON.stringify({}), {headers})
...
export const SystemUserLoginRequest = async ({email, password, otp}: LoginUser) => {
...
export const SystemUserSignupRequest = async({firstName, lastName, email, password, dob}: SystemUser) => {
...
export const UserDelete = async({_id, email, organizationValue}: UserDeleteType)=> {
    ...
    try{
        ...
        const response = await axios.post(URL, JSON.stringify({_id,email,organizationValue}), {headers})
        if(response.data.msg){
...
export const MakeUserAdminReq = async({email, organizationValue}: UserAdminType) => {
    ...
    try{
        ...
        const response = await axios.post(URL, JSON.stringify({email: email, orgName: organizationValue}), {headers})
        if(response.data.msg){
